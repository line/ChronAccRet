...
def print_latex_metrics_neg(metrics):
    vals = [str(x).zfill(2) for x in [1, 2, 3, 5, 10]]
    m2t_keys = [f"m2t/R{i}" for i in vals] + ["m2t/MedR"]
...
def negative_contrastive_metrics(
    ...
    m2t_m, m2t_cols = contrastive_metrics_neg(
...
def contrastive_metrics_neg(
    ...
    #retrieval using motion 
    n, m = sims.shape
    num_queries = n

    dists = -sims
    ids = np.argsort(dists,axis=1)
    print(ids[:,:5])
    np.savetxt('return.csv', ids[:,:5], fmt='%s', delimiter=",")

    sorted_dists = np.sort(dists, axis=1)
    # GT is in the diagonal of the original similarity matrix:
    gt_dists = np.diag(dists[:,:n])[:, None]
